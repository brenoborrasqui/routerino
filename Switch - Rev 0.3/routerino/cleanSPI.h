void SPI_MasterBegin() { //Configura os registradores do SPI
  //Configurando pinos
  DDRB = (1 << PB2) | (1 << PB3) | (0 << PB4) | (1 << PB5); // PB5(MOSI)output, PB4(MISO)input, PB3(SCK)output, PB2(CS)output
  DDRC = (1 << PC2) | (1 << PC3); // PC2(CS)output, PC3(CS)output
  PORTB |= (1 << PB2); //Pino 10 em HIGH
  PORTC |= (1 << PC2) | (1 << PC3); //Pino A2 e A3 em HIGH
...
void COM_start(int x) 
{
  if (x == 0) {
    ...
    PORTC &= ~(1<<PC2);  //Inicia a comunicação SPI escrevendo 0 no pino A1
  } else {
    PORTC &= ~(1<<PC3);  //Inicia a comunicação SPI escrevendo 0 no pino A2
  }
}
...
void COM_end(int x) 
{
  if (x == 0) {
    ...
    PORTC |=  (1<<PC2);   //Finaliza a comunicação SPI escrevendo 1 no pino A2
  } else {
    PORTC |=  (1<<PC3);   //Finaliza a comunicação SPI escrevendo 1 no pino A3
  }
}
